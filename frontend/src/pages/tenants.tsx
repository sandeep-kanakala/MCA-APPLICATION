...
import { useLogout, useUserRole } from '@/app/hooks';
...
export default function Tenantspage() {
  const accounts: Account[] = [
    { id: '1', name: 'Multichoice', path: '/apps/sales/dashboard', logo: 'M' },
  ...
  const navButtons = [{ id: 'multichoice', label: 'Multichoice' }];
              ...
              <div className="w-full lg:w-2/5 bg-white border-b lg:border-b-0 lg:border-r border-gray-200">
                <div className="p-4 border-b border-gray-200 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-gray-900">{firstLetter}</div>
                ...
                <div className="p-3 md:p-6">
                  <div className="space-y-3">
                    {accounts.map((account) => (
                      <Link key={account.id} to={account?.path}>
                        <button
                          key={account.id}
                          className="w-full flex items-center gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors group rounded-lg md:rounded-none"
                        >
